    ...
    if ([self openDB]) {
        ...
        if (sqlite3_prepare_v2(_database, sql, -1, &statement, NULL) != SQLITE_OK) {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
                ...
                if(Pro_name != nil){
                    entity.photos= [NSString stringWithUTF8String:Pro_name];
                }
                
                char * Pro_State   = (char *)sqlite3_column_text(statement,11);
                if(Pro_State != nil){
                    entity.photom= [NSString stringWithUTF8String:Pro_State];
                }
                
                char * Pro_smallpic   = (char *)sqlite3_column_text(statement,12);
                if(Pro_smallpic != nil){
                    entity.photob= [NSString stringWithUTF8String:Pro_smallpic];
                }
                
                char * Pro_bigpic   = (char *)sqlite3_column_text(statement,13);
                if(Pro_bigpic != nil){
                    entity.Dia_Z_weight= [NSString stringWithUTF8String:Pro_bigpic];
                }

                char * Pro_info   = (char *)sqlite3_column_text(statement,14);
                if(Pro_info != nil){
                    entity.Dia_Z_count= [NSString stringWithUTF8String:Pro_info];
                }
                
                char * Pro_goldWeight   = (char *)sqlite3_column_text(statement,15);
                if(Pro_goldWeight != nil){
                    entity.Dia_F_weight= [NSString stringWithUTF8String:Pro_goldWeight];
                }
                
                char * Pro_goldsize   = (char *)sqlite3_column_text(statement,16);
                if(Pro_goldsize != nil)
                {
                    entity.Dia_F_count= [NSString stringWithUTF8String:Pro_goldsize];
                }
